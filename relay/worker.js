...
async function photoRoutes(request, env, url, parts) {
  ...
  // GET /photos.json — the manifest, in the exact shape download_file_list()
  // parses: {"files":[{"path":..,"url":..}]}.
  if (parts[0] === "photos.json") {
    const listed = await env.PHOTOS.list({ prefix: PHOTO_PREFIX });
    const files = listed.keys.map((k) => {
      const path = k.name.slice(PHOTO_PREFIX.length);
      return { path, url: `${url.origin}/photo/${encodeURIComponent(path)}` };
    });
  ...
  if (parts[0] !== "photo" || !parts[1]) return null;      // not ours
  ...
  if (!/^[A-Za-z0-9_-]{1,60}\.bin$/.test(name)) {
  ...
  if (request.method === "PUT" || request.method === "DELETE") {
    ...
    return new Response(JSON.stringify({ path: name, url: `${url.origin}/photo/${encodeURIComponent(name)}` }),
