...
private:
    ...
    esp_lcd_panel_io_handle_t io_handle = nullptr; // stored for runtime commands (setBrightness)
    uint16_t *lineBuffer = nullptr; // 2 rows
...
public:
    ...
    bool setBrightness(uint8_t level); // 0=off, 255=max (register 0x51 WRDISBV)
};
