...
void SD_card_Init(void)
{
  ...
  vTaskDelay(pdMS_TO_TICKS(200)); // Hardware settle delay
  ...
  esp_err_t ret = spi_bus_initialize(SD_SPI, &bus_cfg, SDSPI_DEFAULT_DMA);
  if (ret == ESP_ERR_INVALID_STATE) {
      // SPI bus already initialized, try to reuse or reset
      printf("SPI Bus already initialized, attempting to reuse...\n");
  } else if (ret != ESP_OK) {
  ...
  ret = esp_vfs_fat_sdspi_mount(SDlist, &host, &slot_config, &mount_config, &card);
  if (ret != ESP_OK) {
      printf("SD Mount Failed (0x%x). Retrying with Bus Reset...\n", ret);
