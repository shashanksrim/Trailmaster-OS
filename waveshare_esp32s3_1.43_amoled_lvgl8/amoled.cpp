...
bool Amoled::begin()
{
  ...
  if (esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &io_config, &io_handle) != ESP_OK)
    return false;
  this->io_handle = io_handle; // store for runtime use (setBrightness)
...
bool Amoled::drawArea(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2, uint16_t *bitmap)
{
  ...
  // Direct High-Speed DMA Bypass for even width & height (eliminates slicing and mem-copying!)
  if ((w % 2 == 0) && (h % 2 == 0)) {
      return pushToPanel(x1, y1, bitmap, w, h);
  }
...
bool Amoled::setBrightness(uint8_t level)
{
  // Register 0x51 = WRDISBV. QSPI encoding: opcode(0x02)<<24 | cmd<<8
  // This matches the tx_param() encoding in low_level_amoled.c
  if (!io_handle) return false;
  uint32_t cmd_encoded = (0x02UL << 24) | (0x51UL << 8);
  return esp_lcd_panel_io_tx_param(io_handle, (int)cmd_encoded, &level, 1) == ESP_OK;
}
