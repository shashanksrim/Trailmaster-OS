...
icons = {
    ...
    "settings": {
        "color": "#5a7060",
        "svg": '<svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="{color}" stroke-width="1.5"><path d="M12 15a3 3 0 100-6 3 3 0 000 6z"/><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"/></svg>'
    },
    "about": {
        "color": "#f87171",
...
def generate_c_array(name, img):
    ...
    lines.append(f"const uint8_t ui_img_icon_{name}_map[] = {{")
    for i in range(0, len(data), 12):
        chunk = data[i:i+12]
        lines.append("  " + ", ".join(f"0x{b:02x}" for b in chunk) + ",")
    lines.append("};")
    lines.append(f"const lv_img_dsc_t ui_img_icon_{name} = {{")
    ...
    lines.append(f"  .data = ui_img_icon_{name}_map,")
    lines.append(f"}};")
    
    with open(f"ui_img_icon_{name}.c", "w") as f:
        f.write("\n".join(lines))
...
for name, icon in icons.items():
    ...
    try:
        ...
        renderPM.drawToFile(drawing, "temp.png", fmt="PNG", bg=0)
...
if os.path.exists("temp.svg"): os.remove("temp.svg")
