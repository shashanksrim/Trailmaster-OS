...
void I2C_master_Init(void)
{
  ...
  // Try to install the driver. If it fails (e.g., already installed), just ignore it.
    esp_err_t err = i2c_driver_install(TEST_I2C_PORT, conf.mode, 0, 0, 0);
    if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
        // Only crash if it's a real error, not just "already installed"
        ESP_ERROR_CHECK(err); 
    }
